...
export function deterministicRegime(signals: RegimeSignals): { regime: string; confidence: number } {
  ...
  if (vol > 0.4 && Math.abs(signals.momentum) > 0.3) return { regime: "HIGH_VOL_BREAKOUT", confidence: clamp(0.4 + vol * 0.4, 0.3, 0.8) };
  if (Math.abs(avg) > 0.25 && signals.momentum * avg > 0) return { regime: avg > 0 ? "TREND_UP" : "TREND_DOWN", confidence: clamp(0.4 + Math.abs(avg) * 0.4, 0.3, 0.85) };
